...
-- Executing directly on MomExchange database
...
BEGIN TRY
    ...
    BEGIN
        ...
        WHERE SenderID IN (SELECT UserID FROM Users WHERE UserID <> @KeepUserID)
           OR RecipientID IN (SELECT UserID FROM Users WHERE UserID <> @KeepUserID)
        SET @DeletedMessages = @@ROWCOUNT
        PRINT '  - Da xoa ' + CAST(@DeletedMessages AS VARCHAR) + ' Messages'
    END
    
    -- 2. Delete MilkDonationRequests
    ...
    -- 6. Delete PostC2Cs and related data
    IF EXISTS (SELECT 1 FROM sys.tables WHERE name = 'PostC2Cs')
    BEGIN
        DECLARE @DeletedC2C INT = 0
        DELETE FROM PostC2CExchangePreferences 
        WHERE PostID IN (
            SELECT PostID FROM PostC2Cs 
            WHERE UserID IN (SELECT UserID FROM Users WHERE UserID <> @KeepUserID)
        )
        ...
        WHERE UserID IN (SELECT UserID FROM Users WHERE UserID <> @KeepUserID)
        SET @DeletedC2C = @@ROWCOUNT
        PRINT '  - Da xoa ' + CAST(@DeletedC2C AS VARCHAR) + ' PostC2Cs va du lieu lien quan'
    END
    ...
    BEGIN
        ...
        DELETE FROM Ratings 
        WHERE RaterID IN (SELECT UserID FROM Users WHERE UserID <> @KeepUserID)
